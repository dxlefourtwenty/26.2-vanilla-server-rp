...
execute if predicate jbt.main:unbreaking/mainhand/all if function jbt.main:durability/mainhand/unbreaking run return fail
data modify storage jbt:temp durability.components set from entity @s SelectedItem.components
...
function jbt.main:durability/mainhand/apply with storage jbt:temp durability
execute if score $damage jbt.dummy >= $max_damage jbt.dummy run function jbt.main:durability/mainhand/break
